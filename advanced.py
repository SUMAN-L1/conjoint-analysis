...
if uploaded_file is not None:
    ...
    **Interpretation of the Model Summary:**
    ...
    for item in conjoint_attributes:
        # Get the unique levels for the attribute
        levels = list(np.unique(df[item]))
        nlevels = len(levels)
        
        level_name.append(levels)

        # Slice the coefficients for the current attribute
        begin = end
        end = begin + nlevels - 1

        # Handle part-worth utilities
        if len(model_fit.params[begin:end]) == nlevels - 1:
            new_part_worth = list(model_fit.params[begin:end])
            # Append to ensure part-worth utilities sum to zero
            new_part_worth.append((-1) * sum(new_part_worth))
        else:
            st.write(f"Warning: Mismatch between levels in {item} and model coefficients. Skipping this attribute.")
            continue

        # Store the part-worth utilities
